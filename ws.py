...
def _warn_on_runtime_error(function):
    """Convert expected command failures into warnings without argparse errors."""
    @wraps(function)
    def wrapped():
        try:
            return function()
        except (OSError, ValueError) as error:
            cli_warning(f'ws failed: {type(error).__name__}: {error}')
    return wrapped
...
def run_metric(args):
    """Run the metric command after its arguments have been parsed."""
    from analysis_api import analyze_timeline_batch, analyze_timelines, resolve_timeline_sources
    from analysis_utils import (print_metric_report, print_model_comparison, print_threshold_comparison,
                                save_metric_report)

    def _print_standard_reports(reports, batch, options):
        report_list = reports if isinstance(reports, list) else [reports]
        for idx, rep in enumerate(report_list):
            if idx:
                print()
            if batch:
                predict = rep.get('prediction', {})
                if   predict.get('column') is not None:
                    selector = str(predict['column'])
                elif predict.get('threshold') is not None:
                    selector = f"th={predict['threshold']:g}"
                else:
                    selector = 'N/A'
                print(f"\n=== {rep.get('model', 'N/A')} | {selector} ===")
            print_metric_report(rep, **{**options, 'results_table': 'standard',
                                                   'multi_thresholds': True})

    if args.dirs is not None and args.stream_path:
        raise ValueError('Use positional timeline paths or --dirs, not both')
    if args.dirs is not None:
        stream_inputs = resolve_timeline_sources(args.dirs, directory_list=True)
    elif args.stream_path:
        stream_inputs = list(args.stream_path)
    else:
        raise ValueError('Provide timeline paths or --dirs')
    ...
    if table_mode == 'all':
        ...
        if isinstance(report, list):
            cmp_kwargs = {**print_kwargs, 'results_table': 'thrs_cmp'}
            if is_batch:
                grouped = {}
                for result in report:
                    grouped.setdefault(result.get('model', 'N/A'), []).append(result)
                for mdl_grp in grouped.values():
                    if len(mdl_grp) > 1:
                        print_threshold_comparison( mdl_grp, **cmp_kwargs)
            elif len(report) > 1:
                print_threshold_comparison( report, **cmp_kwargs)
        ...
        if table_mode == 'standard':
            ...
            raise ValueError("'thrs_cmp' is for one timeline directory; use 'models_cmp' for a batch")
...
def main():
    ...
    metric.add_argument('-tbl', '--results-table', nargs='?',
                        choices=('auto', 'standard', 'models_cmp', 'thrs_cmp', 'all'),
                        const='auto', default=None,
                        help='Print/select mode or results table')
