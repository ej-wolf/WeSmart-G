...
# Per-class thresholds for using in events
SINGLE_THRESHOLDS = {
    0: 0.5,   # norm
    1: 0.9,   # abnormal
    2: 0.7,   # fall
...
def choose_single_event(detections):
    """
    detections: list of (cls_id, conf)

    Returns single event: 0,1,2,5 or -1 if none.
    Logic:
...
def choose_grouped_event(detections):
    ...
    Returns grouped event: 3,4 or -1.
    """
    candidates = []
    for cls_id, conf in detections:
...
def parse_sec_str(t):
    """Convert 'HH:MM:SS' or 'MM:SS' or 'SS' to seconds. Returns None if t is empty."""
    if t is None:
        return None
    t = t.strip()
    if t == "":
...
def process_video(video_path, model_path, out_json, step=5, conf_thresh=0.5, if_usual=False, videos_folder='', tension_intervals=[], fight_intervals=[], fall_intervals=[]):
    video_paths = []
    ...
    for video_path in video_paths:
        ...
        if not cap.isOpened():
            print(f"[WARN] Cannot open video (probably corrupted): {video_path}")
            continue #return
        '''if not cap.isOpened():
        ...
        #detections = []
        frame_idx = 0
        #HEAD_IDX      = [0, 1, 2, 3, 4]  # первые 5: голова / лицо
        #SHOULDER_IDX  = [5, 6]           # левое и правое плечо
        THRESH = 0.5
        while True:
            ret, frame = cap.read()
            if not w == 1920:
